...
// CLAMP 2 
  ...
  chassis.pid_wait();
  chassis.pid_odom_set({{-24.5,-53},rev, 85});
  chassis.pid_wait();
  clamps.set(false);
  setIntake(-127);
  ...
  chassis.pid_wait();
  liftPID.target_set(284);
  chassis.pid_turn_set(-89, 110);
  chassis.pid_wait();
  chassis.pid_odom_set({{-58, -3.8}, fwd, 34});
  chassis.pid_wait();
  pros::delay(600);
  setIntake(0);
  liftPID.target_set(2000);
  ...
  chassis.pid_wait();
  chassis.pid_odom_set({{-60, -64}, rev, 80});
  chassis.pid_wait();
  clamps.set(true);
  setIntake(0);
// END OF CORNER 2 
  chassis.pid_odom_set({{-11, 54}, fwd, 127});
  setIntake(0);
  chassis.pid_wait();
  chassis.pid_odom_set({{-64, 60}, fwd, 110});
  setroller(127);
  chassis.pid_wait();
  chassis.pid_odom_set({{0, 51, -90}, rev, 110});
  chassis.pid_wait();
  clamps.set(false);
  pros::delay(200);
  setroller(0);
