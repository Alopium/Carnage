...
//alliance stake 
  ...
  setIntake(-127);
  pros::delay(700);
  chassis.pid_odom_set({{0, -52}, fwd, 115});
...
// CLAMP 2 
  ...
  chassis.pid_wait();
  liftPID.target_set(278);
  chassis.pid_turn_set(-89, 110);
  chassis.pid_wait();
  chassis.pid_odom_set({{-59, -4.1}, fwd, 34});
  chassis.pid_wait();
  pros::delay(600);
  setIntake(0);
  liftPID.target_set(2000);
  ...
  chassis.pid_wait();
  chassis.pid_odom_set({{-58, -66}, rev, 90});
  chassis.pid_wait();
  clamps.set(true);
  setIntake(0);
// END OF CORNER 2 
  chassis.pid_odom_set({{-8, 52}, fwd, 127});
  setIntake(0);
  chassis.pid_wait();
  chassis.pid_odom_set({{-60, 64}, fwd, 120});
  setroller(127);
  pros::delay(3000);
  chassis.pid_odom_set({{0, 49.5, -90}, rev, 110});
  chassis.pid_wait();
  clamps.set(false);
  pros::delay(150);
  setroller(0);
  setIntake(-127);
  chassis.pid_odom_set({{54, 54}, fwd, 127});
  chassis.pid_wait();
  chassis.pid_odom_set({{64, 64}, rev, 127});
  pros::delay(700);
  clamps.set(true);
  chassis.pid_wait();
